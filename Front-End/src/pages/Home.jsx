import { useState, useEffect } from 'react'; // Pastikan useState & useEffect diimport
import { Link } from 'react-router-dom';
import { Search, Star, MapPin } from 'lucide-react';

// Data cadangan agar halaman tidak kosong jika backend mati
const fallbackFields = [
  ...
  // { id: 2, name: 'Royal Synthetic Arena', type: 'Rumput Sintetis', price: '300000', rating: 4.8, image: 'https://images.unsplash.com/photo-1529900748604-07564a03e7a6?auto=format&fit=crop&q=80&w=800' },
];

export default function Home() {
  const [fields, setFields] = useState(fallbackFields); // Default pakai fallback
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/fields?q=${search}&limit=6&page=1`)
      .then(res => res.json())
      .then(res => {
        // Jika backend mengirim data, gunakan itu. Jika tidak, tetap pakai fallback.
        if (res.success && res.data) {
          setFields(res.data);
        }
      })
      .catch(err => {
        console.warn("Backend belum menyala atau koneksi gagal, menggunakan data lokal.");
      });
  }, [search]);
    ...
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {fields.length > 0 ? fields.map(field => (
            ...
            <div className="p-6">
              ...
              <div className="flex justify-between items-center pt-4 border-t border-gray-100 dark:border-gray-800">
                <div>
                  ...
                  <p className="font-bold text-lg">Rp {Number(field.price).toLocaleString()} <span className="text-sm font-normal text-gray-500">/ jam</span></p>
                </div>
          ...
          <p className="text-center col-span-full py-20 text-gray-500">Memuat lapangan...</p>
        )}
      </div>
    </div>
  );
